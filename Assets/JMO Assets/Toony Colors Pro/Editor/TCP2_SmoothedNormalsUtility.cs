...
public class TCP2_SmoothedNormalsUtility : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        */
        ...
        if (mMeshes != null && mMeshes.Count > 0)
        {
            ...
            foreach (var sm in mMeshes.Values)
            {
                ...
                sm.isSkinned = GUILayout.Toggle(sm.isSkinned,
                    new GUIContent("Skinned", "Should be checked if the mesh will be used on a SkinnedMeshRenderer"),
        ...
        TCP2_GUI.Header("Store smoothed normals in:",
            "You will have to select the correct option in the Material Inspector when using outlines", true);
        ...
        mCustomDirectory =
            EditorGUILayout.Toggle(
                new GUIContent("Custom Output Directory", "Save the generated smoothed meshes in a custom directory"),
                mCustomDirectory);
        using (new EditorGUI.DisabledScope(!mCustomDirectory))
        {
            ...
            {
                var outputPath =
                    TCP2_Utils.OpenFolderPanel_ProjectPath(
                        "Choose custom output directory for generated smoothed meshes");
    ...
    private Mesh CreateSmoothedMeshAsset(SelectedMesh originalMesh)
    {
        ...
        var rootPath = mCustomDirectory
            ? Application.dataPath + "/" + mCustomDirectoryPath + "/"
            : TCP2_Utils.FindReadmePath() + OUTPUT_FOLDER;
...
#if UNITY_EDITOR_WIN
        rootPath = rootPath
            .Replace(mCustomDirectory ? Application.dataPath : TCP2_Utils.ToSystemSlashPath(Application.dataPath), "")
            .Replace(@"\", "/");
#else
        ...
        if (originalMeshName.Contains(MESH_SUFFIX)) newAssetName = originalMeshName + ".asset";
        ...
        if (originalMesh.isSkinned)
            newMesh = TCP2_Utils.CreateSmoothedMesh(originalMesh.mesh, mFormat, false, true, false,
                !originalMesh.isAsset || originalMesh.isAsset && assetExists);
        else
            newMesh = TCP2_Utils.CreateSmoothedMesh(originalMesh.mesh, mFormat, mVColors, mTangents, mUV2,
                !originalMesh.isAsset || originalMesh.isAsset && assetExists);
