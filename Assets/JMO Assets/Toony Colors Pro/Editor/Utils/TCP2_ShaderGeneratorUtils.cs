...
public static class TCP2_ShaderGeneratorUtils
{
    ...
    public static string OutputPath
    {
        get
        {
            if (!CustomOutputDir)
            {
                ...
                if (!Directory.Exists(OUTPUT_PATH))
                {
                    ...
                    if (!string.IsNullOrEmpty(rootPath)) return rootPath + "/Shaders 2.0 Generated/";
                }
        ...
        set
        {
            if (_OutputPath != value)
            {
                ...
                foreach (var c in Path.GetInvalidPathChars()) value = value.Replace(c.ToString(), "");
    ...
    public static Shader Compile(TCP2_Config config, Shader existingShader,
        TCP2_ShaderGenerator.ShaderGeneratorTemplate template, bool showProgressBar = true, bool overwritePrompt = true)
    {
        return Compile(config, existingShader, template, showProgressBar ? 0f : -1f, overwritePrompt);
    }

    public static Shader Compile(TCP2_Config config, Shader existingShader,
        TCP2_ShaderGenerator.ShaderGeneratorTemplate template, float progress, bool overwritePrompt)
    {
        //UI
        if (progress >= 0f)
        ...
        //Generate source
        var source = config.GenerateShaderSource(template, existingShader);
    ...
    private static string GenerateShaderSource(this TCP2_Config config,
        TCP2_ShaderGenerator.ShaderGeneratorTemplate template, Shader existingShader = null)
    {
        if (config == null)
        {
        ...
        foreach (var kvp in config.Keywords)
            if (kvp.Key == "UV_mask1")
            {
                config.ToggleFeature("UVMASK1",
                    (kvp.Value == "Independent UV" || kvp.Value == "Independent UV0") && mask1);
                config.ToggleFeature("UVMASK1_UV2", kvp.Value == "Independent UV1" && mask1);
            }
            else if (kvp.Key == "UV_mask2")
            {
                config.ToggleFeature("UVMASK2",
                    (kvp.Value == "Independent UV" || kvp.Value == "Independent UV0") && mask2);
                config.ToggleFeature("UVMASK2_UV2", kvp.Value == "Independent UV1" && mask2);
            }
            else if (kvp.Key == "UV_mask3")
            {
                config.ToggleFeature("UVMASK3",
                    (kvp.Value == "Independent UV" || kvp.Value == "Independent UV0") && mask3);
        ...
        if (!keywords.ContainsKey("SHADER_TARGET"))
        {
            ...
            if (config.shaderTarget == 20) TCP2_Utils.AddIfMissing(features, "FORCE_SM2");
        }
        ...
        for (var i = 0; i < templateLines.Length; i++)
        {
            ...
            {
                //Meta
                if (line.StartsWith("#CONFIG=")) config.configType = line.Substring(8).TrimEnd().ToLower();
                ...
                if (line.StartsWith("#KEYWORDS"))
                {
                    while (i < templateLines.Length)
                    {
                        ...
                        var error = ProcessKeywords(templateLines[i], config, ref features, ref flags, ref keywords,
                            ref i, ref depth, ref stack, ref done);
            ...
            else
            {
                ...
                if (depth >= 0 && stack[depth] || depth < 0) sb.AppendLine(line);
            }
        }

        if (depth >= 0) Debug.LogWarning("[TCP2 Shader Generator] Missing " + (depth + 1) + " ending '///' tags");
    ...
    private static string ProcessKeywords(string line, TCP2_Config config, ref List<string> features,
        ref List<string> flags, ref Dictionary<string, string> keywords, ref int i, ref int depth, ref List<bool> stack,
        ref List<bool> done)
    {
        if (line.Contains("///"))
        {
            ...
            if (depth >= 0 && stack[depth] || depth < 0)
            {
                ...
                switch (parts[0])
                {
                    ...
                    case "disable_kw":
                        ...
                        break;
                    case "enable_flag":
                        TCP2_Utils.AddIfMissing(flags, parts[1]);
                        break;
                    case "disable_flag":
                        TCP2_Utils.RemoveIfExists(flags, parts[1]);
                        break;
    ...
    private static string ProcessCondition(string line, ref List<string> features, ref int i, ref int depth,
        ref List<bool> stack, ref List<bool> done)
    {
        ...
        if (parts.Length == 1) //END TAG
        {
            ...
            {
                var error =
                    "[TCP2 Shader Generator] Found end tag /// without any beginning! Aborting shader generation.\n@ line: " +
                    i;
                Debug.LogError(error);
                return error;
            }
    ...
    private static bool EvaluateExpression(int lineNumber, List<string> features, params string[] conditions)
    {
        ...
        var expression = "";
        for (var n = 2; n < conditions.Length; n++) expression += conditions[n];
    ...
    {
        var str = "";
        foreach (var s in array) str += s + separator;
    ...
    private static string ReplaceKeywords(string line, Dictionary<string, string> searchAndReplace)
    {
        ...
        foreach (var kv in searchAndReplace) line = line.Replace("@%" + kv.Key + "%@", kv.Value);
    ...
    //Save .shader file
    private static Shader SaveShader(TCP2_Config config, Shader existingShader, string sourceCode, bool overwritePrompt,
        bool modifiedPrompt)
    {
        ...
        if (!Directory.Exists(systemPath)) Directory.CreateDirectory(systemPath);

        var fullPath = systemPath + filename + ".shader";
        var overwrite = true;
        if (overwritePrompt && File.Exists(fullPath))
            overwrite = EditorUtility.DisplayDialog("TCP2 : Shader Generation",
                "The following shader already exists:\n\n" + fullPath + "\n\nOverwrite?", "Yes", "No");

        if (modifiedPrompt)
            overwrite = EditorUtility.DisplayDialog("TCP2 : Shader Generation",
                "The following shader seems to have been modified externally or manually:\n\n" + fullPath +
        ...
        if (overwrite)
        {
            ...
            if (SelectGeneratedShader) Selection.objects = new Object[] { shader };
            ...
            if (shaderImporter != null)
            {
                ...
                var textures = new Texture[]
                {
                    AssetDatabase.LoadAssetAtPath<Texture2D>(
                        AssetDatabase.GUIDToAssetPath("af5515bfe14f1af4a9b8b3bf306b9261")),
                    AssetDatabase.LoadAssetAtPath<Texture2D>(
                        AssetDatabase.GUIDToAssetPath("ccad9b0732473ee4e95de81e50e9050f"))
                };
                ...
                customDataList.Add(!string.IsNullOrEmpty(shaderHash)
                    ? shaderHash
                    : shaderImporter.assetTimeStamp.ToString());
            ...
            {
                Debug.LogWarning(
                    "[TCP2 Shader Generator] Couldn't find ShaderImporter.\nMetadatas will be missing from the shader file.");
            }
    ...
    public static void ParseUserData(ShaderImporter importer, out string[] Features, out string[] Flags,
        out Dictionary<string, string> Keywords, out string[] CustomData)
    {
    ...
    {
        var outputPath = OutputPath;
        if (existingShader != null) outputPath = GetExistingShaderPath(config, existingShader);
        ...
        Debug.LogError(
            "[TCP2 Shader Generator] Can't find file 'TCP2_Include.cginc' in project!\nCan't figure out the relative include path to the generated shader.");
        return INCLUDE_REL_PATH;
    }
    ...
    private static void GetFilesSafeInternal(List<string> list, string path, string pattern, bool stopAtFirstMatch)
    {
