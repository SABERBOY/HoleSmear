...
public static class TCP2_Utils
{
    ...
    public static string ToShader(this TextureChannel channel)
    {
        switch (channel)
        {
            ...
            default:
                Debug.LogError("[TCP2_Utils] Unrecognized texture channel: " + channel.ToShader());
                return null;
        }
    }
    ...
    public static TextureChannel FromShader(string str)
    {
        ...
        switch (str)
        {
            ...
            default:
                Debug.LogError(
                    "[TCP2_Utils] Unrecognized texture channel from shader: " + str + "\nDefaulting to Alpha");
                return TextureChannel.Alpha;
        }
    }
    ...
    public static bool ShaderKeywordRadio(string header, string[] keywords, GUIContent[] labels, List<string> list,
        ref bool update)
    {
        var index = 0;
        ...
        for (var i = 0; i < keywords.Length; i++)
        {
            ...
            if (GUI.Toggle(rI, index == i, labels[i],
                    ...
                    i == keywords.Length - 1 ? EditorStyles.miniButtonRight : EditorStyles.miniButtonMid)) index = i;
        }
    ...
    public static int ShaderKeywordRadioGeneric(string header, int index, GUIContent[] labels)
    {
        ...
        for (var i = 0; i < labels.Length; i++)
        {
            ...
            if (GUI.Toggle(rI, index == i, labels[i],
                    i == 0 ? EditorStyles.miniButtonLeft :
                    i == labels.Length - 1 ? EditorStyles.miniButtonRight : EditorStyles.miniButtonMid)) index = i;
        }
    ...
    public static void ShaderVariantUpdate(string feature, List<string> featuresList, List<bool> featuresEnabled,
        bool enable, ref bool update)
    {
    ...
    public static Mesh CreateSmoothedMesh(Mesh originalMesh, string format, bool vcolors, bool vtangents, bool uv2,
        bool overwriteMesh)
    {
...
//			EditorUtility.CopySerialized(originalMesh, newMesh);
        ...
        if (uv2)
        {
            ...
            for (var i = 0; i < newMesh.vertexCount; i++)
            {
                ...
                x = Mathf.Round(x * 15);
                y = Mathf.Round(y * 15);
                var packed = Vector2.Dot(new Vector2(x, y),
    ...
    public static PackedFile[] ExtractArchive(string archivePath, string filter = null)
    {
        ...
        for (var line = 1; line < archiveLines.Length; line++)
        {
            ...
            if (archiveLines[line].StartsWith("#")) break;
            ...
            if (File.Exists(tcpRoot + relativePath)) continue;
