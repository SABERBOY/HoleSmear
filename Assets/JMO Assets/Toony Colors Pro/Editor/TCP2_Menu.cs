...
public static class TCP2_Menu
{
    ...
    private static void UnpackRim()
    {
        UnpackShaders("rim desktop");
    }

    [MenuItem(MENU_PATH + "Unpack Shaders/Rim (Mobile)", false, 800)]
    private static void UnpackRimMobile()
    {
        UnpackShaders("rim mobile");
    }

    [MenuItem(MENU_PATH + "Unpack Shaders/Reflection (Desktop)", false, 800)]
    private static void UnpackReflectionDesktop()
    {
        UnpackShaders("reflection desktop");
    }

    [MenuItem(MENU_PATH + "Unpack Shaders/Matcap (Mobile)", false, 800)]
    private static void UnpackMatcapMobile()
    {
        UnpackShaders("matcap mobile");
    }

    [MenuItem(MENU_PATH + "Unpack Shaders/All Shaders (Mobile)", false, 800)]
    private static void UnpackAllMobile()
    {
        UnpackShaders("mobile");
    }

    [MenuItem(MENU_PATH + "Unpack Shaders/All Shaders (Desktop)", false, 800)]
    private static void UnpackAllDesktop()
    {
        UnpackShaders("desktop");
    }
    ...
    private static void UnpackShaders(string filter)
    {
        ...
        if (string.IsNullOrEmpty(path))
        {
            ...
            Debug.LogError(
                "[TCP2 Unpack Shaders] Couldn't find file: \"TCP2 Packed Shaders.tcp2data\"\nPlease reimport Toony Colors Pro 2.");
            return;
        }
        ...
        if (File.Exists(fullPath))
        {
            ...
            if (files.Length > 8)
                do
                {
                    @continue = EditorUtility.DisplayDialogComplex("TCP2 : Unpack Shaders",
                        "You are about to import " + files.Length +
                        " shaders in Unity.\nIt could take a few minutes!\nContinue?", "Yes", "No", "Help");
                    if (@continue == 2) TCP2_GUI.OpenHelpFor("Unpack Shaders");
                } while (@continue == 2);
            ...
            if (@continue == 0 && files.Length > 0)
            {
                ...
                Debug.Log("Toony Colors Pro 2 - Unpack Shaders:\n" + files.Length +
                          (files.Length > 1 ? " shaders extracted." : " shader extracted."));
                AssetDatabase.Refresh();
            }

            if (files.Length == 0)
                Debug.Log(
                    "Toony Colors Pro 2 - Unpack Shaders:\nNothing to unpack. Shaders are probably already unpacked!");
        }
    }
    ...
    private static bool ResetSelectedMaterials_Validate()
    {
        foreach (var o in Selection.objects)
            if (o is Material)
                return true;
