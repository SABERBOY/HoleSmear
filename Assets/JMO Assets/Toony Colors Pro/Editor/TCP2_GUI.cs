...
public static class TCP2_GUI
{
    ...
    public static Texture2D GetCustomTexture(string name)
    {
        ...
        if (EditorGUIUtility.isProSkin)
            texture = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/" + name + "_Pro.png",
                typeof(Texture2D)) as Texture2D;

        //load default version
        if (texture == null)
            texture = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/" + name + ".png",
                typeof(Texture2D)) as Texture2D;
    ...
    public static GUIStyle ContextualHelpBox
    {
        get
        {
            if (_ContextualHelpBox == null)
            {
                ...
                _ContextualHelpBox.normal.textColor = EditorGUIUtility.isProSkin
        ...
        get
        {
            if (_HeaderDropDownHighlighted == null)
            {
                ...
                var textColor = EditorGUIUtility.isProSkin
                    ...
                    : new Color(0.0f, 0.5f, 0.43f);
                var textColorActive = EditorGUIUtility.isProSkin
                    ? new Color(0.0f, 0.5f, 0.43f)
                    : new Color(0.0f, 0.574f, 0.488f);
    ...
    public static Texture2D SmallHelpIconTexture
    {
        get
        {
            if (_SmallHelpIconTexture == null) _SmallHelpIconTexture = GetCustomTexture("TCP2_SmallHelpIcon");
    ...
    public static void OpenHelpFor(string helpTopic)
    {
        ...
        if (rootDir == null)
        {
            EditorUtility.DisplayDialog("TCP2 Documentation",
                "Couldn't find TCP2 root folder! (the readme file is missing)\nYou can still access the documentation manually in the Documentation folder.",
                "Ok");
        }
        else
        {
            var helpAnchor = helpTopic.Replace("/", "_").Replace(@"\", "_").Replace(" ", "_").ToLowerInvariant() +
                             ".htm";
            var topicLink = "file:///" + rootDir.Replace(@"\", "/") + "/Documentation/Documentation Data/Anchors/" +
                            helpAnchor;
    ...
    {
        if (tooltip != null)
            EditorGUILayout.LabelField(new GUIContent(header, tooltip), HeaderLabel,
    ...
    {
        foldout = EditorGUILayout.Foldout(foldout, guiContent, true,
            highlighted ? HeaderDropDownHighlighted : HeaderDropDown);
        return foldout;
    }
    ...
    {
        if (tooltip != null)
            EditorGUILayout.LabelField(new GUIContent(header, tooltip), SubHeaderLabel,
                GUILayout.ExpandWidth(expandWidth));
        else
    ...
    public static void HeaderAndHelp(string header, string tooltip, string helpTopic)
    {
        ...
        var r = GUILayoutUtility.GetRect(new GUIContent(header, tooltip), EditorStyles.label,
    ...
    {
        if (tooltip != null)
            GUILayout.Label(new GUIContent(header, tooltip), highlight ? EnabledLabel : EditorStyles.label,
    ...
    {
        EditorGUI.LabelField(position, GUIContent.none, new GUIContent(message, GetHelpBoxIcon(msgType)),
            HelpBoxRichTextStyle);
    }

    public static void HelpBoxLayout(string message, MessageType msgType)
    {
        EditorGUILayout.LabelField(GUIContent.none, new GUIContent(message, GetHelpBoxIcon(msgType)),
            HelpBoxRichTextStyle);
    }

    public static void ContextualHelpBoxLayout(string message, bool canHover = false)
    {
        EditorGUILayout.LabelField(GUIContent.none, new GUIContent(message, GetCustomTexture("TCP2_ContextIcon")),
            canHover ? ContextualHelpBoxHover : ContextualHelpBox);
    }
    ...
    public static int RadioChoice(int choice, bool horizontal, params string[] labels)
    {
        ...
        for (var i = 0; i < guiContents.Length; i++) guiContents[i] = new GUIContent(labels[i]);
    ...
    public static int RadioChoice(int choice, bool horizontal, params GUIContent[] labels)
    {
        ...
        for (var i = 0; i < labels.Length; i++)
        {
            ...
            if (GUILayout.Toggle(i == choice, labels[i], style)) choice = i;
        }
    ...
    public static int RadioChoiceHorizontal(Rect position, int choice, params GUIContent[] labels)
    {
        for (var i = 0; i < labels.Length; i++)
        {
            ...
            rI.x += i * rI.width;
            if (GUI.Toggle(rI, choice == i, labels[i],
                    i == 0 ? EditorStyles.miniButtonLeft :
                    i == labels.Length - 1 ? EditorStyles.miniButtonRight : EditorStyles.miniButtonMid)) choice = i;
        }
...
public class TCP2HelpBoxDecorator : MaterialPropertyDrawer
{
    ...
    public override float GetPropertyHeight(MaterialProperty prop, string label, MaterialEditor editor)
    {
        ...
        var height =
            TCP2_GUI.HelpBoxRichTextStyle.CalcHeight(new GUIContent(message, icon),
                Screen.width - (scrollBar ? 51 : 34));
        return height + 6f;
    }
}
...
public class TCP2OutlineNormalsGUIDrawer : MaterialPropertyDrawer
{
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
    {
        TCP2_GUI.Header("OUTLINE NORMALS",
            "Defines where to take the vertex normals from to draw the outline.\nChange this when using a smoothed mesh to fill the gaps shown in hard-edged meshes.");
        ...
        if (EditorGUI.EndChangeCheck()) SetKeyword(prop, index);
    }
    ...
    private int GetCurrentIndex(MaterialProperty prop)
    {
        var index = 0;
    ...
    {
        var label = prop.targets.Length > 1
            ? string.Format("modify Outline Normals of {0} Materials", prop.targets.Length)
            : string.Format("modify Outline Normals of {0}", prop.targets[0].name);
...
public class TCP2Vector4FloatsDrawer : MaterialPropertyDrawer
{
    ...
    public TCP2Vector4FloatsDrawer(string labelX, string labelY, string labelZ, string labelW, float minX, float maxX,
        float minY, float maxY, float minZ, float maxZ, float minW, float maxW)
    {
        labels = new[] { labelX, labelY, labelZ, labelW };
    ...
    private float SignedValue(float val)
    {
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
    {
        ...
        if (expanded)
        {
            ...
            {
                values.x = useSlider
                    ? EditorGUI.Slider(lineRect, labels[0], values.x, min[0], max[0])
                    : EditorGUI.FloatField(lineRect, labels[0], values.x);
                lineRect.y += lineRect.height + spacing;
            }
            ...
            {
                values.y = useSlider
                    ? EditorGUI.Slider(lineRect, labels[1], values.y, min[1], max[1])
                    : EditorGUI.FloatField(lineRect, labels[1], values.y);
                lineRect.y += lineRect.height + spacing;
            }
            ...
            {
                values.z = useSlider
                    ? EditorGUI.Slider(lineRect, labels[2], values.z, min[2], max[2])
                    : EditorGUI.FloatField(lineRect, labels[2], values.z);
                lineRect.y += lineRect.height + spacing;
            }

            if (channelsCount > 3)
                values.w = useSlider
                    ? EditorGUI.Slider(lineRect, labels[3], values.w, min[3], max[3])
                    : EditorGUI.FloatField(lineRect, labels[3], values.w);

            EditorGUIUtility.labelWidth = lw;
        }

        if (EditorGUI.EndChangeCheck()) prop.vectorValue = values;
    }
...
public class TCP2Vector3FloatsDrawer : TCP2Vector4FloatsDrawer
{
    ...
    public TCP2Vector3FloatsDrawer(string labelX, string labelY, string labelZ, float minX, float maxX, float minY,
...
public class TCP2ColorNoAlphaDrawer : MaterialPropertyDrawer
{
    public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
    {
        ...
        var colorValue = EditorGUI.ColorField(position, label, prop.colorValue, true, showAlpha, hdr);
#else
		Color colorValue = EditorGUI.ColorField(position, label, prop.colorValue, true, showAlpha, hdr, null);
#endif
        EditorGUI.showMixedValue = false;
        if (EditorGUI.EndChangeCheck()) prop.colorValue = colorValue;
    }
}
...
{
    private static Texture2D DefaultRampTexture;
    private static bool DefaultTextureSearched; //Avoid searching each update if texture isn't found
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
    {
        ...
        var previewRect = new Rect(position.x + indent, position.y + 1, position.width - indent - 19,
            position.height - 2);
        ...
        if (GUI.Button(buttonRect, "Create New", EditorStyles.miniButtonLeft))
        {
            ...
            {
                var overwriteExistingFile = File.Exists(path);
        ...
        GUI.enabled = assetImporter != null &&
                      (assetImporter.userData.StartsWith("GRADIENT") ||
                       assetImporter.userData.StartsWith("gradient:")) && !prop.hasMixedValue;
        if (GUI.Button(buttonRect, GUI.enabled ? editButtonLabel : editButtonDisabledLabel,
...
public class TCP2KeywordFilterDrawer : MaterialPropertyDrawer
{
    ...
    private bool IsValid(MaterialEditor editor)
    {
        var valid = false;
...
internal class TCP2ToggleNoKeywordDrawer : MaterialPropertyDrawer
{
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
    {
        ...
        else
        {
            ...
            EditorGUI.showMixedValue = false;
            if (EditorGUI.EndChangeCheck()) prop.floatValue = !flag ? 0f : 1f;
        }
    }
}
...
internal class TCP2TextureSingleLine : MaterialPropertyDrawer
{
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
    {
        if (!IsPropertyTypeSuitable(prop))
        {
            EditorGUI.HelpBox(position, "TextureSingleLine used on a non-texture property: " + prop.name,
                MessageType.Warning);
        }
        else
        {
