...
public class TCP2_MaterialInspector : ShaderGUI
{
    ...
    public override void AssignNewShaderToMaterial(Material material, Shader oldShader, Shader newShader)
    {
        ...
        if (shaderImporter != null)
        {
            ...
            if (mShaderFeatures.Count > 0 && mShaderFeatures[0] == "USER") isGeneratedShader = true;
        }
    }
    ...
    private void UpdateFeaturesFromShader()
    {
        if (targetMaterial != null && targetMaterial.shader != null)
        {
            ...
            var shaderImporter =
                AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(targetMaterial.shader)) as ShaderImporter;
            if (shaderImporter != null)
            {
//				mShaderFeatures = new List<string>(shaderImporter.userData.Split(new string[]{","}, System.StringSplitOptions.RemoveEmptyEntries));
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        if (!string.IsNullOrEmpty(mVariantError))
        {
            ...
            EditorGUILayout.HelpBox(
                "Some of the shaders are packed to avoid super long loading times when you import Toony Colors Pro 2 into Unity.\n\n" +
        ...
        if (materialEditor.isVisible && !mShader.hasMultipleDifferentValues && mShader.objectReferenceValue != null)
        {
            ...
            if (ShowFilteredProperties(null, props))
            {
                if (!isGeneratedShader)
                    TCP2_Utils.ShaderKeywordToggle("TCP2_DISABLE_WRAPPED_LIGHT", "Disable Wrapped Lighting",
                        "Disable wrapped lighting, reducing intensity received from lights", keywordsList,
                        ref updateKeywords, "Disable Wrapped Lighting");
            ...
            if (CategoryFilter("TEXTURE_RAMP"))
            {
                ...
                {
                    if (TCP2_Utils.ShaderKeywordToggle("TCP2_RAMPTEXT", "Texture Toon Ramp",
                            "Make the toon ramp based on a texture", keywordsList, ref updateKeywords, "Ramp Style"))
                        ShowFilteredProperties("#RAMPT#", props);
                    else
            ...
            if (CategoryFilter("BUMP"))
            {
                ...
                {
                    if (TCP2_Utils.ShaderKeywordToggle("TCP2_BUMP", "BUMP/NORMAL MAPPING",
                            "Enable bump mapping using normal maps", keywordsList, ref updateKeywords,
                            "Normal/Bump map")) ShowFilteredProperties("#NORM#", props);
                }
            ...
            if (CategoryFilter("SPECULAR", "SPECULAR_ANISOTROPIC"))
            {
                ...
                else
                {
                    ...
                    TCP2_Utils.ShaderVariantUpdate("Specular", ShaderVariants, ShaderVariantsEnabled, specular,
                        ref updateVariant);

                    specular |= TCP2_Utils.ShaderKeywordRadio("SPECULAR",
                        new[] { "TCP2_SPEC_OFF", "TCP2_SPEC", "TCP2_SPEC_TOON" }, new[]
                        {
                    ...
                    if (specular)
                    {
                        ...
                        if (specr) ShowFilteredProperties("#SPECT#", props);
                    }
                }
            ...
            if (CategoryFilter("REFLECTION") && !isMobileShader)
            {
                if (isGeneratedShader)
                {
                    ...
                    if (HasFlags("REFL_ROUGH"))
                    {
                        ...
                        EditorGUILayout.HelpBox("Cubemap Texture needs to have MipMaps enabled for Roughness to work!",
                            MessageType.Info);
                    }
                }
                else
                {
                    ...
                    TCP2_Utils.ShaderVariantUpdate("Reflection", ShaderVariants, ShaderVariantsEnabled, reflection,
                        ref updateVariant);

                    reflection |= TCP2_Utils.ShaderKeywordRadio("REFLECTION",
                        new[] { "TCP2_REFLECTION_OFF", "TCP2_REFLECTION", "TCP2_REFLECTION_MASKED" }, new[]
                        {
                            new GUIContent("Off", "No Cubemap Reflection"),
                            new GUIContent("Global", "Global Cubemap Reflection"),
                            new GUIContent("Masked",
                                "Masked Cubemap Reflection (using the main texture's alpha channel)")
                        },
                        keywordsList, ref updateKeywords);
            ...
            if (CategoryFilter("MATCAP"))
            {
                ...
                else if (isMobileShader)
                {
                    ...
                    TCP2_Utils.ShaderVariantUpdate("Matcap", ShaderVariants, ShaderVariantsEnabled, matcap,
                        ref updateVariant);

                    matcap |= TCP2_Utils.ShaderKeywordRadio("MATCAP", new[] { "TCP2_MC_OFF", "TCP2_MC", "TCP2_MCMASK" },
                        new[]
                        {
                    ...
                    if (matcap) ShowFilteredProperties("#MC#", props);
            ...
            if (CategoryFilter("RIM", "RIM_OUTLINE"))
            {
                if (isGeneratedShader)
                {
                    ...
                    if (HasFlags("RIMDIR"))
                    {
                        ...
                        if (HasFlags("PARALLAX"))
                            EditorGUILayout.HelpBox(
                                "Because it affects the view direction vector, Rim Direction may distort Parallax effect.",
                                MessageType.Warning);
                    }
                }
                else
                {
                    ...
                    TCP2_Utils.ShaderVariantUpdate("Rim", ShaderVariants, ShaderVariantsEnabled, rim,
                        ref updateVariant);
                    TCP2_Utils.ShaderVariantUpdate("RimOutline", ShaderVariants, ShaderVariantsEnabled, rimOutline,
                        ref updateVariant);

                    rim |= rimOutline |= TCP2_Utils.ShaderKeywordRadio("RIM",
                        new[] { "TCP2_RIM_OFF", "TCP2_RIM", "TCP2_RIMO" }, new[]
                        {
                    ...
                    if (rim || rimOutline)
                    {
                        ...
                        if (CategoryFilter("RIMDIR"))
                            if (TCP2_Utils.ShaderKeywordToggle("TCP2_RIMDIR", "Directional Rim",
                                    "Enable directional rim control (rim calculation is approximated if enabled)",
                                    keywordsList, ref updateKeywords))
                                ShowFilteredProperties("#RIMDIR#", props);
                    }
                }
            ...
            if (CategoryFilter("OUTLINE", "OUTLINE_BLENDING"))
            {
                ...
                else
                {
                    ...
                    TCP2_Utils.ShaderVariantUpdate("Outline", ShaderVariants, ShaderVariantsEnabled, hasOutlineOpaque,
                        ref updateVariant);
                    TCP2_Utils.ShaderVariantUpdate("OutlineBlending", ShaderVariants, ShaderVariantsEnabled,
                        hasOutlineBlending, ref updateVariant);

                    hasOutline |= TCP2_Utils.ShaderKeywordRadio("OUTLINE",
                        new[] { "OUTLINE_OFF", "OUTLINES", "OUTLINE_BLENDING" }, new[]
                        {
                ...
                if (hasOutline)
                {
                    ...
                    {
                        var outlineTextured = TCP2_Utils.ShaderKeywordToggle("TCP2_OUTLINE_TEXTURED",
                            "Outline Color from Texture",
                            "If enabled, outline will take an averaged color from the main texture multiplied by Outline Color",
                            keywordsList, ref updateKeywords);
                        if (outlineTextured) ShowFilteredProperties("#OUTLINETEX#", props);
                    }

                    TCP2_Utils.ShaderKeywordToggle("TCP2_OUTLINE_CONST_SIZE", "Constant Size Outline",
                        "If enabled, outline will have a constant size independently from camera distance",
                        keywordsList, ref updateKeywords);
                    if (TCP2_Utils.ShaderKeywordToggle("TCP2_ZSMOOTH_ON", "Correct Z Artefacts",
                            "Enable the outline z-correction to try to hide artefacts from complex models",
                            keywordsList, ref updateKeywords)) ShowFilteredProperties("#OUTLINEZ#", props);
                    ...
                    EditorGUI.indentLevel++;
                    TCP2_Utils.ShaderKeywordRadio(null,
                        new[]
                        {
                    ...
                    if (hasOutlineBlending)
                    {
                        ...
                        {
                            TCP2_GUI.Header("OUTLINE BLENDING",
            ...
            if (CategoryFilter("ALPHA", "CUTOUT") && isGeneratedShader)
            {
                ...
                if (cutout) ShowFilteredProperties("#CUTOUT#", props);
            }
    ...
    {
        if (!isGeneratedShader) return true;
    ...
    private bool ShowFilteredProperties(string filter, MaterialProperty[] properties, bool indent = true)
    {
        ...
        foreach (var p in properties)
            if ((p.flags & (MaterialProperty.PropFlags.PerRendererData | MaterialProperty.PropFlags.HideInInspector)) ==
