...
public class TCP2_OutlineInspector : MaterialEditor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (targetMaterial.shader != mCurrentShader) mCurrentShader = targetMaterial.shader;
        ...
        if (isVisible && !mShader.hasMultipleDifferentValues && mShader.objectReferenceValue != null)
        {
            ...
            if (ShowFilteredProperties(null, props, false)) TCP2_GUI.Separator();
            ...
            if (!mShaderModel2)
            {
                var texturedOutline = TCP2_Utils.ShaderKeywordToggle("TCP2_OUTLINE_TEXTURED",
                ...
                if (texturedOutline) ShowFilteredProperties("#OUTLINETEX#", props);
            }

            TCP2_Utils.ShaderKeywordToggle("TCP2_OUTLINE_CONST_SIZE", "Constant Size Outline",
                "If enabled, outline will have a constant size independently from camera distance", keywordsList,
                ref updateKeywords);
            if (TCP2_Utils.ShaderKeywordToggle("TCP2_ZSMOOTH_ON", "Correct Z Artefacts",
                    "Enable the outline z-correction to try to hide artefacts from complex models", keywordsList,
                    ref updateKeywords)) ShowFilteredProperties("#OUTLINEZ#", props);
            ...
            TCP2_Utils.ShaderKeywordRadio(null,
                new[] { "TCP2_NONE", "TCP2_COLORS_AS_NORMALS", "TCP2_TANGENT_AS_NORMALS", "TCP2_UV2_AS_NORMALS" }, new[]
                {
