...
public class TCP2_GradientManager
{
    ...
    public static bool CreateAndSaveNewGradientTexture(int width, string unityPath)
    {
        ...
        gradient.colorKeys = new[]
            { new GradientColorKey(Color.black, 0.45f), new GradientColorKey(Color.white, 0.55f) };
    ...
    public static string GradientToUserData(params Gradient[] gradients)
    {
        ...
        foreach (var gradient in gradients)
        {
            ...
            for (var i = 0; i < gradient.colorKeys.Length; i++)
                output += string.Format("{0},{1},", ColorToHex(gradient.colorKeys[i].color),
                    gradient.colorKeys[i].time);
            output = output.TrimEnd(',');
            output += ";";
            for (var i = 0; i < gradient.alphaKeys.Length; i++)
                output += string.Format("{0},{1},", gradient.alphaKeys[i].alpha, gradient.alphaKeys[i].time);
            output = output.TrimEnd(',');
#if UNITY_5_5_OR_NEWER
...
#if UNITY_5_5_OR_NEWER
        if (keys.Length >= 4) gradient.mode = (GradientMode)Enum.Parse(typeof(GradientMode), keys[3]);
#endif
    }
    ...
    public static List<Gradient> GetGradientsFromUserData(string userData)
    {
        ...
        try
        {
            ...
            var legacy = userData.StartsWith("GRADIENT");
        ...
        catch (Exception e)
        {
            EditorApplication.Beep();
            Debug.LogError(
                "[TCP2_GradientManager] Couldn't parse gradients from texture\nMake sure the texture was created with the Ramp Generator.\nError:\n" +
                e.ToString());
            return null;
        }
    }
    ...
    public static Color[] GetPixelsFromGradient(Gradient gradient, int width, int height, Color[] pixels)
    {
        for (var x = 0; x < width; x++)
        {
            ...
            for (var i = 0; i < height; i++) pixels[x + i * width] = col;
        }
    ...
    public static void SetPixelsFromGradients(Texture2D texture, Gradient[] gradients, int width, int height)
    {
        ...
        var colorCount = gradients[0].colorKeys.Length;
        var alphaCount = gradients[0].alphaKeys.Length;
        ...
        for (var i = 1; i < gradients.Length; i++)
        {
            ...
            {
                Debug.LogError(
                    "[TCP2 Ramp Generator] Invalid Gradients: gradients need to have the same number of color/alpha keys to be interpolated properly.");
                return;
            }
        ...
        var blockHeight = Mathf.FloorToInt(height / (float)(gradients.Length - 1));
        var lerpGradient = new Gradient();
        var pixelsBuffer = new Color[width];
        for (var y = 0; y < height; y++)
        {
            var l = y % blockHeight / (float)blockHeight;

            var rampIndex = Mathf.FloorToInt(y / (float)height * (gradients.Length - 1));
            var nextRampIndex = rampIndex + 1;

            var g1 = gradients[rampIndex];
            var g2 = gradients[nextRampIndex];

            var colorKeys = new GradientColorKey[g1.colorKeys.Length];
            for (var i = 0; i < g1.colorKeys.Length; i++)
            ...
            for (var i = 0; i < g1.alphaKeys.Length; i++)
    ...
    private static Gradient Deserialize(string serializedGradient)
    {
        // format:
        ...
        for (var i = 0; i < colorKeysStr.Length; i += 2)
            colorKeys.Add(new GradientColorKey(HexToColor(colorKeysStr[i]), float.Parse(colorKeysStr[i + 1])));

        // parse alpha keys:
        var alphaKeysStr = split[1].Split(',');
        for (var i = 0; i < alphaKeysStr.Length; i += 2)
            alphaKeys.Add(new GradientAlphaKey(float.Parse(alphaKeysStr[i]), float.Parse(alphaKeysStr[i + 1])));
