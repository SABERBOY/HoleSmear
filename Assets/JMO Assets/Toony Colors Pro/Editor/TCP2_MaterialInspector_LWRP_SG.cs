...
{
    //Properties
    private Material targetMaterial => materialEditor == null ? null : materialEditor.target as Material;
    private Stack<bool> toggledGroups = new Stack<bool>();
    private bool firstTimeApply;
    ...
    {
        var surfaceTypeValue = (int)surfaceTypeProp.floatValue;
        ...
        {
            var blendModeValue = (int)blendModeProp.floatValue;
            EditorGUI.BeginChangeCheck();
        ...
        EditorGUI.BeginChangeCheck();
        var twoSidedEnabled = EditorGUILayout.Toggle(Styles.twoSidedLabel, culling.floatValue == 0);
        if (EditorGUI.EndChangeCheck())
            culling.floatValue = twoSidedEnabled ? 0 : 2;

        EditorGUI.BeginChangeCheck();
        var alphaClipEnabled = alphaClip.floatValue == 1;
        if (alphaClipEnabled)
        {
        ...
        if (EditorGUI.EndChangeCheck()) alphaClip.floatValue = alphaClipEnabled ? 1 : 0;

        EditorGUILayout.Space();
    }

    public new void ShaderPropertiesGUI(Material material)
    {
        EditorGUI.BeginChangeCheck();
        {
            DoSurfaceArea();
        ...
        if (materialEditor.isVisible && !mShader.hasMultipleDifferentValues && mShader.objectReferenceValue != null)
        {
            ...
            foreach (var p in properties)
            {
                ...
                if (p.name.StartsWith("__BeginGroup"))
                {
                    ...
                    {
                        GUILayout.Space(8f);
                        p.floatValue = EditorGUILayout.Foldout(p.floatValue > 0, p.displayName, TCP2_GUI.FoldoutBold)
                            ? 1
                            : 0;
                    }
