        ...
        private void OnGUI()
        {
            ...
            if (GUI.changed)
            {
                ...
                if (editMode)
                {
                    ...
                    {
                        TCP2_GradientManager.SetPixelsFromGradients(linkedTexture, m2dGradients, linkedTexture.width,
            ...
            if (saveButton)
            {
                var path = EditorUtility.SaveFilePanel("Save Generated Ramp", TCP2_GradientManager.LAST_SAVE_PATH,
