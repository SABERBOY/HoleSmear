...
public partial class TCP2_ShaderGenerator : EditorWindow
{
    ...
    public class ShaderGeneratorTemplate
    {
        ...
        public void FeaturesGUI(TCP2_Config config)
        {
            ...
            for (var i = 0; i < length; i++) uiFeatures[i].DrawGUI(config);
        }
        ...
        private TextAsset LoadTextAsset(string filename)
        {
            var asset = AssetDatabase.LoadAssetAtPath(
                string.Format("Assets/JMO Assets/Toony Colors Pro/Editor/Shader Templates/{0}", filename),
        ...
        if (Directory.Exists(systemPath))
        {
            ...
            foreach (var sysPath in txtFiles)
            {
                ...
                if (TCP2_Utils.SystemToUnityPath(ref unityPath))
                {
                    ...
                    if (textAsset != null && !list.Contains(textAsset)) list.Add(textAsset);
                }
            }
    ...
    private void OnEnable()
    {
        ...
        if (allTemplates != null && allTemplates.Length > 0)
        {
            ...
            foreach (var item in menuItemsSorted)
                loadTemplateMenu.AddItem(new GUIContent(item[0] as string), false, OnLoadTemplate,
                    item[1] as TextAsset);
        }
    }

    private void OnLoadTemplate(object textAsset)
    {
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        var _tmpTemplate =
            EditorGUILayout.ObjectField("Template:", Template.textAsset, typeof(TextAsset), false) as TextAsset;
        if (_tmpTemplate != Template.textAsset) Template.SetTextAsset(_tmpTemplate);
        //Load template
        ...
        */
        ...
        mCurrentShader =
            EditorGUILayout.ObjectField("Current Shader:", mCurrentShader, typeof(Shader), false) as Shader;
        ...
        if (mCurrentConfig == null) NewShader();

        if (mCurrentConfig.isModifiedExternally)
            EditorGUILayout.HelpBox(
                "It looks like this shader has been modified externally/manually. Updating it will overwrite the changes.",
                MessageType.Warning);
        ...
        EditorGUI.BeginChangeCheck();
        mCurrentConfig.ShaderName = EditorGUILayout.TextField(
            new GUIContent("Shader Name", "Path will indicate how to find the Shader in Unity's drop-down list"),
            mCurrentConfig.ShaderName);
        mCurrentConfig.ShaderName = Regex.Replace(mCurrentConfig.ShaderName, @"[^a-zA-Z0-9 _!/]", "");
        if (EditorGUI.EndChangeCheck() && sAutoNames) mCurrentConfig.AutoNames();
		...
		if(mCurrentShader != null)
		{
			...
			{
				mCurrentShaderImporter =
 ShaderImporter.GetAtPath(AssetDatabase.GetAssetPath(mCurrentShader)) as ShaderImporter;
			}
        ...
        GUILayout.FlexibleSpace();
        GUI.color = mDirtyConfig ? gColor * unsavedChangesColor : GUI.color;
        if (GUILayout.Button(mCurrentShader == null ? "Generate Shader" : "Update Shader", GUILayout.Width(120f),
                GUILayout.Height(30f)))
        {
            if (Template == null)
            {
                EditorUtility.DisplayDialog("TCP2 : Shader Generation",
                    "Can't generate shader: no Template file defined!\n\nYou most likely want to link the TCP2_User.txt file to the Template field in the Shader Generator.",
                    "Ok");
                return;
            }

            //Set config type
            if (Template.templateType != null) mCurrentConfig.configType = Template.templateType;

            //Set config file
            mCurrentConfig.templateFile = Template.textAsset.name;

            var generatedShader =
                TCP2_ShaderGeneratorUtils.Compile(mCurrentConfig, mCurrentShader, Template, true, !sOverwriteConfigs);
            ReloadUserShaders();
        ...
        GUILayout.BeginHorizontal();
        sSelectGeneratedShader = GUILayout.Toggle(sSelectGeneratedShader,
            new GUIContent("Select Generated Shader", "Will select the generated file in the Project view"),
            GUILayout.Width(180f));
        sAutoNames = GUILayout.Toggle(sAutoNames,
            new GUIContent("Automatic Name", "Will automatically generate the shader filename based on its UI name"),
            GUILayout.ExpandWidth(false));
        GUILayout.EndHorizontal();
        GUILayout.BeginHorizontal();
        sOverwriteConfigs = GUILayout.Toggle(sOverwriteConfigs,
            new GUIContent("Always overwrite shaders", "Overwrite shaders when generating/updating (no prompt)"),
            GUILayout.Width(180f));
        sHideDisabled = GUILayout.Toggle(sHideDisabled,
            new GUIContent("Hide disabled fields", "Hide properties settings when they cannot be accessed"),
            GUILayout.ExpandWidth(false));
        GUILayout.EndHorizontal();

        EditorGUILayout.BeginHorizontal();
        EditorGUI.BeginChangeCheck();
        TCP2_ShaderGeneratorUtils.CustomOutputDir = GUILayout.Toggle(TCP2_ShaderGeneratorUtils.CustomOutputDir,
            new GUIContent("Custom Output Directory:",
                "Will save the generated shaders in a custom directory within the Project"), GUILayout.Width(165f));
        ...
        if (TCP2_ShaderGeneratorUtils.CustomOutputDir)
        {
            ...
            {
                var outputPath =
                    TCP2_Utils.OpenFolderPanel_ProjectPath("Choose custom output directory for TCP2 generated shaders");
                if (!string.IsNullOrEmpty(outputPath)) TCP2_ShaderGeneratorUtils.OutputPath = outputPath;
            }
        }
        else
        {
        ...
        if (EditorGUI.EndChangeCheck()) ReloadUserShaders();

        GUI.enabled = sGUIEnabled;
        EditorGUILayout.EndHorizontal();

        EditorGUI.BeginChangeCheck();
        sLoadAllShaders = GUILayout.Toggle(sLoadAllShaders,
            new GUIContent("Reload Shaders from all Project",
                "Load shaders from all your Project folders instead of just Toony Colors Pro 2.\nEnable it if you move your generated shader files outside of the default TCP2 Generated folder."),
            GUILayout.ExpandWidth(false));
        if (EditorGUI.EndChangeCheck()) ReloadUserShaders();
    ...
    {
        ReloadUserShaders();
        if (mCurrentShader == null && mConfigChoice != 0) NewShader();
    }
    ...
    private void LoadUserPrefs()
    {
        ...
        TCP2_ShaderGeneratorUtils.CustomOutputDir =
            EditorPrefs.GetBool("TCP2_TCP2_ShaderGeneratorUtils.CustomOutputDir", false);
    ...
    private void SaveUserPrefs()
    {
        ...
        EditorPrefs.SetBool("TCP2_TCP2_ShaderGeneratorUtils.CustomOutputDir",
            TCP2_ShaderGeneratorUtils.CustomOutputDir);
    ...
    private void LoadCurrentConfig(TCP2_Config config, bool loadConfigTemplate = true)
    {
        ...
        mDirtyConfig = false;
        if (sAutoNames) mCurrentConfig.AutoNames();
        mCurrentHash = mCurrentConfig.ToHash();
        mFirstHashPass = false;
    ...
    private void LoadCurrentConfigFromShader(Shader shader)
    {
        ...
        else
        {
            ...
            ShowNotification(new GUIContent(
                "Invalid shader loaded: it doesn't seem to have been generated by the TCP2 Shader Generator!"));
            mCurrentShader = null;
            NewShader();
        }
    }
    ...
    private void ReloadUserShaders()
    {
        ...
        if (mCurrentShader != null) mConfigChoice = mUserShadersLabels.IndexOf(mCurrentShader.name);
        ...
        for (var i = 0; i < mUserShaders.Length; i++)
            if (mUserShaders[i] != null)
                loadShadersMenu.AddItem(new GUIContent(mUserShaders[i].name), false, OnLoadShaderFromMenu,
                    mUserShaders[i]);
    }

    private void OnLoadShaderFromMenu(object shaderObj)
    {
    ...
    private string[] GetShaderLabels(Shader[] array, string firstOption = "New Shader")
    {
        ...
        foreach (var shader in array) labelsList.Add(shader.name);
