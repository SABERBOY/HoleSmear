    ...
    private void Update()
    {
        ...
        if (dragging)
        {
            ...
            if (dragging)
            {
                ...
                transform.localScale = new Vector3(Mathf.Clamp(lsx, 0.1f, Mathf.Infinity),
                    Mathf.Clamp(lsy, 0.1f, Mathf.Infinity), Mathf.Clamp(lsz, 0.1f, Mathf.Infinity));


                foreach (var uvs in gameObject.GetComponentsInChildren<UVScaler>()) uvs.SetUV();
                foreach (var uvs in gameObject.GetComponentsInChildren<ScaleToColor>())
                    uvs.SetColor(Vector3.Magnitude(transform.localScale) / scaleM);
            }
