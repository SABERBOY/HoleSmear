...
using MTGOfferWall 	 		= MintegraliOSOfferWall;
...
using MTGOfferWall 			= MintegralAndroidOfferWall;
...
public class Mintegral
{
	...
	private static Dictionary<string, MTGOfferWall> _offerWallPluginsDict =
		new Dictionary<string, MTGOfferWall> ();
	...
	{
		foreach (MTGOfferWallInfo offerWallAdInfo in offerWallAdInfos) {
			if (!_offerWallPluginsDict.ContainsKey (offerWallAdInfo.adUnitId)) {
				_offerWallPluginsDict.Add (offerWallAdInfo.adUnitId, new MTGOfferWall (offerWallAdInfo));
			}
		}
	}
	...
	public static void initMTGSDK(string appId,string apiKey){ 
		Debug.LogWarning ("mintegral: initMTGSDK \n" + "------------------------------");
	...
	{ 
		MTGOfferWall plugin;
		if (_offerWallPluginsDict.TryGetValue (adUnitId, out plugin)) {
			plugin.requestOfferWallAd ();
		} else {
			Debug.LogWarning (String.Format (ADUNIT_NOT_FOUND_MSG, adUnitId));
		}
	}		
